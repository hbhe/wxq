...
class Util 
{
    ...
    static public function getSessionGhid() {
        ...
        if (!empty($gh_id = \Yii::$app->request->get('gh_id'))) {
            ...
            Yii::$app->session->set('gh_sid', $gh_sid);        
            ...
            $gh = \common\models\WxGh::findOne(['sid' => $_SERVER['gh_sid']]);   
            return $gh->gh_id;                                    
            ...
            $gh = \common\models\WxGh::findOne(['sid' => $gh_sid]);    
            return $gh->gh_id;      
        } else if (!empty($gh_id = \Yii::$app->user->identity->gh_id)) {
            return $gh_id;    
        } else {
            //yii::warning('No gh_id, ' . __METHOD__);        
            return null;
         }            
    }
    ...
    public static function forwardWechatXML($url, $token, $xml) {
        ...
        yii::error(['ready to forward', $url, $token, $xml]);
        $response = \common\wosotech\Util::curl_core($url, $xml);
        yii::error(['forward done', $response]);             	        	    
        return $response;
    }
