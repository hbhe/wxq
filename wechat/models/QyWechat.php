...
class QyWechat
{
    ...
    public function valid($return=false)
    {
        ...
        if (!isset($array[0]) || ($array[0] != 0)) {
            if (!$return) {
                $this->log(print_r([__LINE__, __METHOD__, $array, $encryptStr, $this->appid, $this->encodingAesKey], true));            
                die('解密失败！');
            } else {
                return false;
            }
    ...
    public function setRevData(array $receive)
    {
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    public function createMenu($data,$agentid=''){
        ...
        if ($result)
        {
            ...
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                $this->log(print_r([__METHOD__, __LINE__, $result, $json], true));                            
                return false;
            }
            return true;
        }
    ...
    */     
    ...
    {
        if (!empty($this->suit_access_token)) {
            return $this->suit_access_token;
        }
        
        $key = __METHOD__ . $this->appid;        
        if ($this->suit_access_token = $this->getCache($key))  {
            return $this->suit_access_token;
        }

        $suite_ticket = $this->getSuiteTicket();
        $data = ['suite_id' => $this->appid, 'suite_secret' => $this->appsecret, 'suite_ticket' => $suite_ticket];
        $result = $this->http_post(self::API_URL_PREFIX . '/service/get_suite_token', self::json_encode($data));
        if (!$result) {
        ...
        $expire = $json['expires_in'] ? intval($json['expires_in']) - 100 : 3600;
        $this->setCache($key, $this->suit_access_token, $expire);
        return $this->suit_access_token;
    }
    ...
    public function getPreAuthCode() 
    {
        ...
        $data = ['suite_id' => $this->appid];
        $result = $this->http_post(self::API_URL_PREFIX . "/service/get_pre_auth_code?suite_access_token=$suite_access_token", self::json_encode($data));
        if (!$result) {
            $this->log(print_r([__METHOD__, __LINE__, $result, $data], true));
            exit;
        }
        ...
        $expire = $json['expires_in'] ? intval($json['expires_in']) - 100 : 1200;
        $this->setCache($key, $json['pre_auth_code'], $expire);
        return $json['pre_auth_code'];
    }

    /*
    Array (
        [access_token] => M-BKY7zUqHHiRfwDK8f8fzh1J8c07K_gqxTEpjEfpzZUl_Jq5ojKGD3S1tDnA12Z
        [expires_in] => 7200
        [permanent_code] => fg5YUTWVaY8fLTCtQ3erEuEdtQ7B-ES3bIz0FB4GsdqfeiYLfYkSwOd7-hRDqlET
        [auth_corp_info] => Array
            (
                [corpid] => wxe675e8d30802ff44
                [corp_name] => my corp name
                [corp_type] => unverified
                [corp_round_logo_url] => http://mmbiz.qpic.cn/mmbiz_png/pKXBq1NedKdiaLwoP7hsyRgHFyCCMoRTo2Ofe1fqHrkvvw0feutKKEHAnUJQ75ggTtw8wjpSpI2ZUhqdDTCXib9Q/0
                [corp_square_logo_url] => http://wx.qlogo.cn/mmhead/Q3auHgzwzM6EVz1qjZiblNYHlePq6ZPZNicqpZjSgG1CcNhOTdyddrfQ/0
                [corp_user_max] => 200
                [corp_agent_max] => 30
                [corp_wxqrcode] => http://shp.qpic.cn/bizmp/YI2BzCzzDnan6TTdrmiaB3CB1Ipe8gQYicytpDJ4b9PSAbK9omqhKllQ/
                [corp_full_name] => my corp name
                [subject_type] => 1
            )
                    ...
                    (
                        [0] => Array
                            (
                                [agentid] => 8
                                [name] => 会议管理
                                [square_logo_url] => /cgi-bin/redirect?url=http%3A%2F%2Fmmbiz.qpic.cn%2Fmmocbiz%2Fu9IrCb4Nfuj97BBv9USI82lKN8iaKnPpqJlIcmMFpXKiakMqO2eMeO2g%2F
                                [round_logo_url] =>
                                [appid] => 1
                                [api_group] => Array
                                    (
    ...
    */
    ...
    {
        $suite_access_token = $this->getSuiteToken();
        $data = ['suite_id' => $this->appid, 'auth_code' => $auth_code];
        $result = $this->http_post(self::API_URL_PREFIX . "/service/get_permanent_code?suite_access_token=$suite_access_token", self::json_encode($data));
        if (!$result) {
            $this->log(print_r([__METHOD__, __LINE__, $result, $data], true));
            return false;        
        }
        ...
        $this->log(print_r(['try getPermanentCode ....', __METHOD__, __LINE__, $json], true));
    ...
    public function getSuiteAccessToken($auth_corpid, $permanent_code) 
    {    
        $suite_access_token = $this->getSuiteToken();
        $data = ['suite_id' => $this->appid, 'auth_corpid' => $auth_corpid, 'permanent_code' => $permanent_code];
        $key = [__METHOD__ , $data];        
        if ($value = $this->getCache($key))  {
            return $value;
        }
        
        $result = $this->http_post(self::API_URL_PREFIX . "/service/get_corp_token?suite_access_token=$suite_access_token", self::json_encode($data));
        if (!$result) {
            $this->log(print_r([__METHOD__, __LINE__, $result, $data], true));
            return false;        
        }
        ...
        $this->setCache($key, $json['access_token'], $expire);
    ...
    [
        ...
        'agentlist' => [
            [
                'agentid' => 16,
                'name' => '会议管理',
                'square_logo_url' => 'http://wx.qlogo.cn/mmhead/Q3auHgzwzM51icic7kRc31Y5cAkk9nibicsVsoavthYlfS36vHbr14zbFw/0',
                'round_logo_url' => 'http://mmbiz.qpic.cn/mmbiz_png/BVgkeUw5pa0J6BibvqAdkchMJynmhQVmP3uc6zb3LTKHcdic9f16sp7tV3tl1iaXTNoDGwIWHSN641gaErz0tomEA/0',
            ],
        ],
    ]
    */
    public function getAgentList() {
        if (!$this->access_token && !$this->checkAuth()) {
            $this->log(print_r([__METHOD__, __LINE__], true));          
            exit;        
        }
        ...
        if (!$result) {
            $this->log(print_r([__METHOD__, __LINE__, $result, $this->access_token], true)); 
            exit;
        }
    ...
    [
        ...
        'close' => 0,
        'description' => '参会消息下发, 会议签到',
        'redirect_domain' => 'wxe-admin.buy027.com',
        'agentid' => 16,
    ...
    */
    public function getAgent($agentid) {
        ...
        if (!$result) {
            $this->log(print_r([__METHOD__, __LINE__, $result, $this->access_token, $agentid], true)); 
            exit;
        }
...
class Prpcrypt
{
    ...
    public function decrypt($encrypted, $appid)
    {
        ...
        try {
            ...
            \yii::error(print_r([__LINE__, __METHOD__, $encrypted, $appid, $e], true)); \yii::$app->end();                             
        ...
        try {
            ...
            if (strlen($result) < 16) {
                \yii::error(print_r([__LINE__, __METHOD__, $encrypted, $appid, $result], true)); \yii::$app->end();    
                return "";
            }
            ...
            //\yii::error(print_r([__LINE__, __METHOD__, $from_appid, $appid, $result], true));                                
        } catch (Exception $e) {
            //print $e;
            \yii::error(print_r([__LINE__, __METHOD__, $encrypted, $appid, $e], true)); \yii::$app->end();           
        ...
        // 自建应用的才需要做检查; 如果是 套件开发, appid是套件id, from_appid是使用者的corp_id, 二者并不相等
       /* 
        if ($from_appid != $appid) {
            \yii::error(print_r([__LINE__, __METHOD__, $from_appid, $appid, $result], true));                    
